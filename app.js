...
const supabase = createClient(
    ...
    'eyJhbGciOiJIUzI1NiIsInR5cCI6ImFub24iLCJpYXQiOjE3ODc1NTg4NDksImV4cCI6MjEwMzEzNDg0OX0.6a5FljFNpGkfYw0hPQ133QjgYc1jM28mkiaJYTfuCvk'
);
...
if (requestForm) {
    ...
    const loadRequests = async () => {
        ...
        requestList.innerHTML = data.length ? data.map((request) => `<article class="rounded border border-orange-200 bg-orange-50 p-4"><p class="font-bold text-orange-900">${request.service}</p><p class="text-sm text-gray-600">${request.location}</p><p class="mt-2 text-sm">Status: <strong>${request.status}</strong></p>${request.agent_id && request.agent ? `<div class="mt-3 border-t border-orange-200 pt-3 text-sm text-green-700"><p class="font-semibold">Agent: ${request.agent.full_name}</p><p>Email: <a class="underline" href="mailto:${request.agent.email}">${request.agent.email}</a></p><p>Phone: <a class="underline" href="tel:${request.agent.phone}">${request.agent.phone || 'Not provided'}</a></p></div>` : ''}</article>`).join('') : '<p class="text-gray-600">You have not made a request yet.</p>';
    };
    await loadRequests();
